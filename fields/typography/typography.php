...
class WPSFramework_Option_typography extends WPSFramework_Options {
	...
	public function output() {
		...
		if (is_object ( $google_json )) {
			...
			echo '<select name="' . $this->element_name ( '[family]' ) . '" class="' . $is_chosen . $chosen_rtl . 'wpsf-typo-family" data-atts="family"' . $this->element_attributes () . '>';
			...
			if (! empty ( $is_variant )) {
				...
				echo '<select name="' . $this->element_name ( '[variant]' ) . '" class="' . $is_chosen . $chosen_rtl . 'wpsf-typo-variant" data-atts="variant">';
