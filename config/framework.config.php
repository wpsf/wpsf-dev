    ...
    array(
      ...
      'title' => 'Textarea Field',
		...
		array(
			...
			'title' => 'Number',
			'default' => '10',
			'after' => ' <i class="wpsf-text-muted">$ (dollars)</i>'
		) ,
		array(
			'id' => 'image_select_1',
			'type' => 'image_select',
	...
	'sections' => array(
		...
		array(
			...
			'fields' => array(
				...
				array(
					...
					'after' => ' <i class="wpsf-text-muted">( this option is optional )</i>'
				) ,
				array(
					'id' => 'unique_text_7',
					'type' => 'text',
					'title' => 'Text Field with Before-text',
					'before' => '<i class="wpsf-text-muted">( important )</i> '
				) ,
				array(
					'id' => 'unique_text_8',
					'type' => 'text',
					'title' => 'Text Field with After-block-text',
					'after' => '<p class="wpsf-text-info">Information: There is some description for option.</p> '
				) ,
				array(
					'id' => 'unique_text_9',
					'type' => 'text',
				...
				array(
					...
					'after' => '<p class="wpsf-text-muted">There is custom css <strong>style="width: 175px; height: 40px; border-color: #93C054;"</strong></p>'
				) ,
				array(
					'id' => 'unique_text_12',
					'type' => 'text',
					...
					'after' => '<p class="wpsf-text-muted">There is custom css <strong>style="width: 100%;"</strong></p>'
				) ,
				array(
					'id' => 'unique_text_13',
					'type' => 'text',
					'before' => '<h4>Text Field without left title</h4>',
					'after' => '<p class="wpsf-text-muted">This Text Field do not using left title, just using before text here. also you can remove it.</h4>'
				)
			)
		) , // end: fields
		...
		array(
			...
			'fields' => array(
				...
				array(
					...
					'after' => '<p class="wpsf-text-muted">Information: There is some description for option.</p> '
				) ,
				array(
					'id' => 'unique_textarea_7',
					'type' => 'textarea',
					'title' => 'Textarea Field with Before-text',
					'before' => '<p class="wpsf-text-muted">Information: There is some description for option.</p> '
				) ,
				array(
					...
					'after' => '<p class="wpsf-text-muted">custom textarea attribute rows=10</p> '
				) ,
				array(
					'id' => 'unique_textarea_13',
					'type' => 'textarea',
					'before' => '<h4>Textarea Field without left title</h4>',
					'after' => '<p class="wpsf-text-muted">This Textarea Field do not using left title, just using before text here. also you can remove it.</h4>'
				)
			)
		) ,
		...
		array(
			...
			'fields' => array(
				...
				array(
					...
					'type' => 'checkbox',
					'title' => 'Checkbox With iCheck',
					'label' => 'Yes, Please. (iCheck)',
					'class' => 'icheck',
				) ,
				array(
				...
				array(
					...
					'after' => '<p class="wpsf-text-muted"><strong>query_args:</strong> sort_order=desc, sort_column=post_title</p>'
				) ,
				...
				array(
					...
					'after' => '<div class="wpsf-text-muted"><strong>query_args:</strong> post_type=your_post_type_name</div>'
				) ,
				array(
					...
					'after' => '<div class="wpsf-text-muted"><strong>query_args:</strong> post_type=your_post_type_name, taxonomy=your_taxonomy_name</div>'
				) ,
				array(
					...
					'after' => '<div class="wpsf-text-muted"><strong>query_args:</strong> taxonomies=your_taxonomy_name, order=asc, orderby=name</div>'
				)
			)
		...
		array(
			...
			'fields' => array(
                ...
                array(
					'id' => 'unique_radio_icheck',
					'type' => 'radio',
					'title' => 'Radio Field (iCheck)',
					'options' => array(
						'yes' => 'Yes, Please.',
						'no' => 'No, Thank you.'
					),
				...
				array(
					...
					'after' => '<div class="wpsf-text-muted">Reference site about Lorem Ipsum, a random Lipsum generator.</div>'
				) ,
				array(
					'id' => 'unique_radio_4',
					'type' => 'radio',
		...
		array(
			...
			'fields' => array(
				...
				array(
					...
					'after' => ' <i class="wpsf-text-muted">(px)</i>'
				)
			)
		) , // end: number options
		...
		array(
			...
			'fields' => array(
				...
				array(
					...
					'after' => '<p class="wpsf-text-muted">Lets write some description for this icon field.</i>'
				)
			)
		) , // end: icon options
		...
		array(
			...
			'fields' => array(
				...
				array(
					...
					'after' => '<p class="wpsf-text-muted">You can use default value <strong>get_template_directory_uri()</strong>."/images/screenshot-1.png"</p>'
				) ,
				array(
					'id' => 'unique_upload_6',
					'type' => 'upload',
		...
		array(
			...
			'fields' => array(
				...
				array(
					...
					'after' => '<p class="wpsf-text-warning">This field using debug=true</p>',
					'debug' => true
				)
			)
		)
...
$wpsf_demo_settings[] = array(
	...
	'fields' => array(
		...
		array(
			...
			'type' => 'text',
			'title' => 'Requried Text',
			'after' => ' <small class="wpsf-text-warning">( * required )</small>',
			'default' => 'lorem ipsum',
			'validate' => 'required'
		) ,
		array(
			'type' => 'notice',
			'class' => 'info',
			'content' => 'Also you can add your own validate from <strong>themename/wpsf-framework/functions/validate.php</strong>'
		)
	)
);
...
$wpsf_demo_settings[] = array(
	...
	'fields' => array(
		array(
			...
			'after' => '<p class="wpsf-text-muted">This text field sanitized already, without any settings. we are using wordpress core.<br /> for eg. try too add <strong>&lt;p></strong> html tag</p>'
		) ,
		array(
			'id' => 'sanitie_text_2',
			'type' => 'text',
			'title' => 'Disable Sanitized Text',
			'after' => '<p class="wpsf-text-muted">Disabled sanitize for this field (sanitize=false). try too add <strong>&lt;p></strong> html tag so, you can write anything</p>',
			'sanitize' => false
		) ,
		array(
			...
			'after' => '<p class="wpsf-text-muted">This textarea field sanitized already, without any settings. we are using wordpress core.<br /> just allowing this tags wp core $allowedposttags</p>'
		) ,
		array(
			'id' => 'sanitie_textarea_2',
			'type' => 'textarea',
			'title' => 'Disabled Sanitized Textarea',
			'after' => '<p class="wpsf-text-muted">Disabled sanitize for this field (sanitize=false). you can write anything</p>',
			'sanitize' => false
		) ,
		array(
			'type' => 'notice',
			'class' => 'info',
			'content' => 'Custom Sanitize, Also you can add your own validate from <strong>themename/wpsf-framework/functions/sanitize.php</strong>'
		) ,
		array(
			'id' => 'sanitie_text_1',
			'type' => 'text',
			'title' => 'Custom Sanitize Text',
			'after' => '<p class="wpsf-text-muted">This text field sanitized as slug title (sanitize="title")</p>',
			'sanitize' => 'title'
		)
	)
);
...
$wpsf_demo_settings[] = array(
	...
	'fields' => array(
		...
		array(
			'type' => 'content',
			'content' => 'WordPress-Settings-Framework Framework is <strong>free</strong> to use both personal and commercial. If you used commercial, <strong>please credit</strong>. Read more about <a href="http://www.gnu.org/licenses/gpl-2.0.txt" target="_blank">GNU License</a>'
		)
	)
);