...
class WPSFramework_Settings extends WPSFramework_Abstract {
    ...
    public function find_current_sections(){
        ...
        $parent_sections = current($this->options);

        $child_sections = isset($parent_sections['sections']) ? current($parent_sections['sections']) : false;
        $child_sections = ($child_sections !== false) ? $child_sections['name'] : false;        
        $child_sections = (!empty($cache['section_id'])) ? $cache['section_id'] : $child_sections;
        $this->current_section = wpsf_get_var('wpsf_section_id',$child_sections);
        
        $parent_sections = (is_array($parent_sections) && isset($parent_sections['name'])) ? $parent_sections['name'] : false;
        $parent_sections = (!empty($cache['parent_section_id'])) ? $cache['parent_section_id'] : $parent_sections;
        $this->parent_sectionid = wpsf_get_var('wpsf-parent-section',$parent_sections);
        
        if(isset($this->options[$this->current_section]) && isset($this->options[$this->parent_sectionid])){
	...
	public function validate_save($request) {
		$add_errors = array ();
        $section_id = wpsf_get_var('wpsf_section_id',false);
        $parent_sectionid = wpsf_get_var('wpsf-parent-section',false);
		...
		// set transient
		$transient_time = (wpsf_language_defaults () !== false) ? 30000 : 10000;
		set_transient ( '_wpsf_' . $this->get_cache_key(), array (
				'errors' => $add_errors,
				'section_id' => $section_id,
            'parent_section_id' => $parent_sectionid,
		), $transient_time );
    ...
    public function get_settings_fields() {
        ...
        $parent = $this->current_section('parent');
        echo '<input class="wpsf-parent-section" type="hidden" name="wpsf-parent-section" value="'.$parent.'" /> ';
		return $this->catch_output ( false );
	}
    ...
    public function validate_sections(){
        $parentsection = $this->current_section('parent');
        
        if($parentsection !== false){
            foreach($this->options as $option){
                if($option['name'] == $parentsection){
    ...
    public function admin_page() {
		...
		$this->load_template ( 'settings/render.php', array (
				'class' => &$this,
				'errors' => $errors_html,
				'current_section_id' => $this->current_section 
		) );
	}
		...
		if ($this->theme () === 'modern') {
			...
			foreach ( $this->options as $key => $tab ) {
				if ((isset ( $tab ['sections'] ))) {
                    $tab_active = array();
                    if($tab['name'] == $this->current_section('parent')){
                        $tab_active = wpsf_array_search ( $tab ['sections'], 'name', $this->current_section );    
                    }
					...
					foreach ( $tab ['sections'] as $tab_section ) {
						$active_tab = ($this->current_section == $tab_section ['name'] && $this->current_section('parent') == $tab['name']) ? ' class="wpsf-section-active"' : '';
						$icon = (! empty ( $tab_section ['icon'] )) ? '<i class="wpsf-icon ' . $tab_section ['icon'] . '"></i>' : '';
						$r .= '<li><a href="'.$this->get_tab_url($tab_section['name'],$tab['name']).'"' . $active_tab . ' data-parent-section="'.$tab['name'].'" data-section="' . $tab_section ['name'] . '">' . $icon . $tab_section ['title'] . '</a></li>';
					}
					$r .= '</ul>';
					$r .= '</li>';
				} else {
					...
					if (isset ( $tab ['fields'] )) {
                        ...
                        if($this->current_section('parent') === false || $this->current_section('parent') == $this->current_section() ){
                            $active_list = ($this->current_section == $tab ['name']) ? ' class="wpsf-section-active"' : '';    
                        }
						
                        $r .= '<li><a href="'.$this->get_tab_url($tab['name']).'"' . $active_list . ' data-section="' . $tab ['name'] . '" data-parent-section="' . $tab ['name'] . '">' . $icon . $tab ['title'] . '</a></li>';
					} else {
            ...
            foreach($this->options as $key => $tab){
                ...
                $tab_active = '';
                if(isset($tab ['sections'])){
                    if($this->current_section('parent') == $tab['name']){
                        $tab_active = wpsf_array_search ( $tab ['sections'], 'name', $this->current_section );    
                    }                    
                }
                ...
                if(!empty($tab_active)){
                    ...
                    $is_active = ($this->current_section == $tab['name']) ? ' nav-tab-active ' : '';
                }
    ...
    public function html_modern_theme() {
        ...
        foreach( $this->sections as $section ) {
            if($this->is_single_page() === false){
                if($this->current_section != $section['name']){
                    continue;
                }
            }
            
            if( isset( $section['fields'] ) || isset($section['callback_hook']) ) {
                
                if($this->current_section('parent') !== false){
                    if(isset($section['page_id']) && $section['page_id'] == $this->current_section('parent')){
                        $active_content = ( $this->current_section == $section['name'] ) ? ' style="display: block;"' : '';
                    } else {
                        $active_content = '';
                    }
                }
                ...
                $div_id = 'wpsf-tab-';
                $div_id .= isset($section['page_id']) ? $section['page_id'].'-' : '';
                $div_id .= $section['name'];
                
                $r .= '<div id="'.$div_id.'" class="wpsf-section"'. $active_content .'>';
    ...
    public function html_simple_theme(){
        ...
        foreach($this->options as $options){
            ...
            if($this->is_single_page() === false){
                if($this->current_section != $options['name']){
                    continue;
                }                
            }
            ...
            if(isset($options['sections'])){
                ...
                $first_section = current($options['sections']);
                $first_section = $first_section ['name'];
                foreach($options['sections'] as $section){
                    $fields = '';
                    if($main_active !== true){
                        $main_active = ($this->current_section == $section['name'] && $this->current_section('parent') == $options['name']) ? true : false;
                    }

                    if($this->current_section == $section['name'] && $this->current_section('parent') == $options['name']){
                        $is_csmenu = 'current';
                    ...
                    $l1_html .= '<div id="wpsf-tab-'.$options['name'].'-'.$section['name'].'" class="wpsf-section" '.$is_html_active.'>';
                ...
                if($main_active === false){
                    ...
                    $l1_html = str_replace('id="wpsf-tab-'.$options['name'].'-'.$first_section.'"',' id="wpsf-tab-'.$options['name'].'-'.$first_section.'" style="display:block;" ',$l1_html);
                }
                ...
                $main_active = ($this->current_section == $options['name']) ? '' : ' style="display:none;" ';
            
            } else if(isset($options['fields'])){
                $is_active = ($this->current_section == $options['name']) ? '' : ' style="display:none;" ';
            ...
            if($this->is_single_page() === false && $this->current_section == $options['name']){
